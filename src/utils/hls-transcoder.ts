...
class HlsTranscoder {
  ...
  public async startStream(
    ...
    if (sessionId) {
      for (const [key, session] of this.activeStreams.entries()) {
        if (session.sessionId === sessionId && key !== streamKey) {
          console.log(
            `[HLS] Stopping previous stream ${key} for session ${sessionId}`,
          );
          this.stopStream(key);
        }
      }
    ...
    console.log(
      `[HLS] Starting transcoding for ${streamKey} (Start: ${startTime || 0}s)`,
    );
    ...
    ffmpeg.stderr.on("data", (data) => {
      ...
      if (
        ...
        if (msg.includes("404 Not Found") || msg.includes("403 Forbidden")) {
          console.log(
            `[HLS] Critical error detected for ${streamKey}, stopping stream.`,
          );
    ...
    ffmpeg.on("close", (code) => {
      console.log(
        `[HLS] FFmpeg process for ${streamKey} exited with code ${code}`,
      );
      if (this.activeStreams.has(streamKey)) {
  ...
  public stopStream(cmd: string) {
    ...
    if (session) {
      ...
      setTimeout(() => {
        fs.promises
          .rm(streamDir, { recursive: true, force: true })
          .catch((err) =>
            console.error(`[HLS] Failed to clean dir ${streamDir}`, err),
          );
      }, 1000);
    }
  }
  ...
  public async startVODStream(
    ...
    const ffmpegArgs = [
      ...
      "index.m3u8",
      "-hls_segment_filename",
      path.join(streamDir, "stream_%v_data%03d.ts"),
    ];
